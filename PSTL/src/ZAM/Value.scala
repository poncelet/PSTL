...
class Zamblock (tag : BlockT.blockT, size : Long, value : Array[Value] ) extends Value {
  
  def getval = value
  def getsize = size
  def gettag = tag
  
  def at(n : Int) = value(n) 
  ...
  override def toString = tag match {
    ...
    case BlockT.double_t => {var str = "double de " + size + " : "
    						value.foreach(ent => str += ent.asInstanceOf[Zamint].getval + ",")
    						str}
    
    case BlockT.doublearray_t => {var str = "tableau de double de " + size + " : "
    							 value.foreach(ent => str += ent + ", ")
    							 str}
